...
const ProtectPdf = () => {
  ...
  const generateThumbnail = async (file) => {
    try {
      ...
      reader.onload = async function () {
        ...
        const viewport = page.getViewport({ scale: 0.5 }); // Thumbnail size scale
  ...
  const protectPdf = async () => {
    ...
    try {
      // API URL logic - ensures it hits Render or Localhost correctly
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Server failed to protect PDF.");
      }
  ...
  const resetAll = useCallback(() => {
    if (downloadUrl) URL.revokeObjectURL(downloadUrl); // Clean up memory
    setFile(null);
    setThumbnail(null);
    setPassword("");
    setConfirm("");
    ...
    <div className="max-w-6xl mx-auto pt-32 pb-16 px-4">
      ...
      <header className="text-center mb-12">
        ...
        <p className="text-lg text-slate-500 max-w-2xl mx-auto">
          Secure your PDF documents with professional AES encryption. Fast, private, and 100% free.
        </p>
      </header>
        ...
        <div className="rounded-3xl p-8 flex flex-col items-center justify-center bg-gray-50/50 min-h-[400px] transition-all overflow-hidden relative border-2 border-dashed border-gray-200">
          {downloadUrl ? (
            <div className="flex flex-col items-center justify-center animate-in zoom-in duration-300 text-center">
              ...
              <p className="text-slate-500 mt-2">Your file is now encrypted and secured.</p>
            ...
            <div className="relative group animate-in zoom-in duration-300">
              <img src={thumbnail} className="max-h-[420px] rounded-lg shadow-2xl border-[12px] border-white" alt="PDF Preview" />
              <button onClick={resetAll} className="absolute -top-4 -right-4 bg-rose-500 text-white p-2 rounded-full shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:scale-110">
                <Trash2 size={20} />
              </button>
            </div>
          ) : (
        ...
        <div className="space-y-6 bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 bottom-4 text-gray-400 hover:text-gray-600">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full p-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-rose-500 transition-all outline-none font-medium"
              />
              <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-4 bottom-4 text-gray-400 hover:text-gray-600">
          ...
          {error && (
            <div className="flex items-center gap-2 text-red-500 bg-red-50 p-3 rounded-xl text-sm font-medium animate-shake">
          ...
          <div className="flex flex-col gap-3 pt-4">
            ...
            >
              {loading ? (
                <>
                  <LoaderIcon className="animate-spin mr-2" /> Encrypting...
                </>
              ) : "Protect PDF Document"}
            </button>
            <button onClick={resetAll} className="flex items-center justify-center gap-2 text-gray-400 font-semibold py-2 hover:text-rose-500 transition-colors">
              <Trash2 size={16} /> Reset Fields
            </button>
          </div>
      ...
      <section className="mt-20 max-w-4xl mx-auto">
        <div className="border border-gray-100 rounded-3xl overflow-hidden bg-white shadow-sm">
          <button onClick={() => setIsGuideOpen(!isGuideOpen)} className="w-full flex items-center justify-between p-6 text-left hover:bg-gray-50 transition-colors group">
          ...
          {isGuideOpen && (
            <div className="p-8 pt-0 border-t border-gray-50 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-center mt-6">
                {["Upload PDF", "Set Password", "Protect File", "Download"].map((step, idx) => (
                  <div key={idx} className="space-y-3">
                    <div className="w-10 h-10 bg-rose-100 text-rose-600 rounded-full flex items-center justify-center mx-auto font-bold">{idx + 1}</div>
                    <p className="text-gray-600 text-sm font-medium leading-relaxed">{step}</p>
                  </div>
                ))}
              </div>
            </div>
