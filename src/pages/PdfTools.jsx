...
import { 
    ...
    LayoutGrid, Tag, Globe, FileEdit, FileX, Unlock, 
    PenTool, FileSpreadsheet, Presentation
} from "lucide-react";
...
const ToolCard = ({ to, icon: ToolIcon, title, desc, color }) => (
    ...
    >
        <Link 
            to={`/${to}`} 
            className="group relative block p-5 bg-white/80 backdrop-blur-md border border-gray-100/50 rounded-2xl shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 overflow-hidden h-full"
        >
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {tools.map((tool, index) => (
                        <ToolCard key={index} to={tool.path} icon={tool.icon} title={tool.title} desc={tool.desc} color={tool.color} />
                    ))}
                </div>
            </div>
        </div>
